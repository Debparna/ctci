...
bool isUniqueChars2(const string& str)
{
  ...
  bool arr[256] = { false }; //create an array
...
bool anagram1(char *str1, char *str2)
{
  ...
  // Sort both strings
  quickSort(str1, 0, n1 - 1); //-1 coz of NULL char in end
...
int partition(char A[], int startIndex, int endIndex)
{
  ...
  exchange (&A[i + 1], &A[endIndex]); //Pivot is last element so swap after partition is over
  return (i + 1); //return partition array index
}
