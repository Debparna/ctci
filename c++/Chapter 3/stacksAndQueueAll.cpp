...
//while popping, if the one being popped from s1 is equal to min the pop the top from s2 since if popped it will no longer be min. Same with push if the one being pushed is lesser than s2 top then that is the new min so push it in s2.
...
//Q3
//SetOfStacks - //create a stack and when it exceeds capacity beyond the threshold, create a new stack and place item in it
class newStack{
private:
  stack<int> *myStack;
  int threshold;
