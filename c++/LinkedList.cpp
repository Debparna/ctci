...
void LinkedList::removeDuplicates_1(Node* head)
{
  ...
  map<int, bool> map; //typedef pair<key_type, data_type> value_type;
  map[head->data] = 1; //set first one to true

  while (head->next) //while the node after head exists
  {
    if (map[head->next->data] == 0) //if the next data is false
    {
      map[head->next->data] = 1; //it is not the same therefore no duplicate
      head = head->next; //move to next
    }
    else //else the next node is true
    {
      Node* temp = head->next; //save the next duplicate node in temp
      cout << "Duplicate Found: " << temp->data << " removed" << endl; //display data of duplicate node
      delete temp; //delete duplicate node
      head->next = head->next->next; //set next to node after duplicate/deleted node
    }
  }
}


// “current” does a normal iteration, while “runner” iterates through all prior nodes to check for dups. Runner will only see one dup per node, because if there were multiple duplicates they would have been removed already
...
void LinkedList::removeDuplicates_2(Node* head)
{
  ...
  Node* runner;
  while (head) //while current which starts at head exists
  {
    runner = head; //set current to runner
    while (runner->next) //while runner next exists
    {
      ...
      if (head->data == runner->next->data)// if  duplicate then
      {
    ...
    head = head->next; // move on  to next node to check if it has a duplicate in the whole list
  }//end of while1
}
...
// Copy data from next node into current node and then delete the next node.
...
bool LinkedList::removeNode(Node* node){
  if(node == NULL && node->next == NULL){
    ...
    //no deletion but just create a temp node which overrides the temp->next node
    Node* temp;
    temp = node->next;
    node->data = temp->data;
    node->next = temp->next;
...
//imagine two pointer, if they move at different speeds, they will collide.
//We can do this on a linked list, one moves two nodes for every time the other moves one node.
//If there is a loop they will collide, otherwise they will get to the end of the list.
...
//Iterative reverse with 3 pointers
void interativeReverse(Node *head){
  Node *curr = new Node;
  Node *prev = new Node;
  Node *next = new Node;
...
int main()
{
  ...
  //List.removeDuplicates_1(List.head);
  //cout << "The list with duplicated removed is: ";
  //List.display();

  //cout << "Nth to end node is " << List.findNthFromEnd(head, 3) << endl;
